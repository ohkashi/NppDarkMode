...
namespace NppDarkMode
{
	...
	{
		const auto& nppTheme = NppTheme::getInstance();
		Options opt;
		opt.enable = nppTheme._darkmode._isEnabled;
		opt.enableMenubar = opt.enable;
		opt.enablePlugin = nppTheme._darkmode._isEnabledPlugin;

		g_colorToneChoice = nppTheme._darkmode._colorTone;
		tCustom.change(nppTheme._darkmode._customColors);
	...
	void initDarkMode(bool bWindowMode)
	{
		_options = configuredOptions();

		initExperimentalDarkMode();
		initAdvancedOptions();
		g_advOptions._enableWindowsMode = bWindowMode;
		...
		{
			auto& nppTheme = NppTheme::getInstance();
			nppTheme._darkmode._isEnabled = NppDarkMode::isDarkModeReg() && !IsHighContrast();
			_options.enable = nppTheme._darkmode._isEnabled;
			_options.enableMenubar = _options.enable;
			if (_options.enable) {
				nppTheme.setCurrentDefaultBgColor(getDarkerBackgroundColor());
				nppTheme.setCurrentDefaultFgColor(getDarkerTextColor());
			}
		}
	...
	{
		const auto& nppTheme = NppTheme::getInstance();
		g_advOptions = nppTheme._darkmode._advOptions;
	}
	...
	{
		auto& nppTheme = NppTheme::getInstance();
		auto& advOpt = nppTheme._darkmode._advOptions;
	...
	static COLORREF g_treeViewBg = NppTheme::getInstance().getCurrentDefaultBgColor();
	...
	void subclassAndThemeListView(HWND hwnd, NppDarkModeParams p)
	{
		...
		ListView_SetTextColor(hwnd, NppTheme::getInstance().getCurrentDefaultFgColor());
		ListView_SetTextBkColor(hwnd, NppTheme::getInstance().getCurrentDefaultBgColor());
		ListView_SetBkColor(hwnd, NppTheme::getInstance().getCurrentDefaultBgColor());
	...
	{
		TreeView_SetTextColor(hwnd, NppTheme::getInstance().getCurrentDefaultFgColor());
		TreeView_SetBkColor(hwnd, NppTheme::getInstance().getCurrentDefaultBgColor());
	...
	{
		COLORREF bgColor = NppTheme::getInstance().getCurrentDefaultBgColor();
...
NppTheme* NppTheme::getInstancePointer() noexcept
{
	static NppTheme g_instance;
	return &g_instance;
};
